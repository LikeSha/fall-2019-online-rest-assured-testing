...
public class Spartan {
    ...
    // above constructor, why we don't set id inside constructor ? because :we are not allowed to set id when we send
    //POST request to sever.id is auto generated by sever, so when we send this json body to post new Spartan,
    //id can not be specified, id will be automatically assigned by server.
